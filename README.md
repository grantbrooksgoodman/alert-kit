...
[`TranslationDelegate`](Sources/Protocols/TranslationDelegate.swift) requires a single method, `getTranslations(_:languagePair:hud:timeout:)`, which translates the given inputs for a specified language pair and throws an error on failure. When no translation delegate is registered, AlertKit falls back to the default implementation provided by [Translator](https://github.com/grantbrooksgoodman/translator).
